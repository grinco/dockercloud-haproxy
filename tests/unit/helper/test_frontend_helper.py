...
class FrontendHelperTestCase(unittest.TestCase):
    ...
    def test_config_common_part_with_ssl(self, mock_get_bind_string):
        ...
        self.assertEqual(
            ["bind :443 ssl crt /certs/", "http-request add-header X-Forwarded-Proto https",
             "acl is_websocket hdr(Upgrade) -i WebSocket"],
            frontend_section)
        self.assertFalse(monitor_uri_configured)
    ...
    def test_config_common_part_with_extra_frontend_sttings(self, mock_get_bind_string):
        mock_get_bind_string.return_value = ("80", False)
        frontend_helper.EXTRA_FRONTEND_SETTINGS = {'80': ["http-request add-header header1 value1", "http-request add-header header2 value2"]}
        frontend_section, monitor_uri_configured = config_common_part("80", "ssl crt /certs/", [])
        self.assertEqual(["bind :80", 'http-request add-header X-Forwarded-Proto http',
                          'maxconn 55555',
                          'http-request add-header header1 value1',
                          'http-request add-header header2 value2',
                          "acl is_websocket hdr(Upgrade) -i WebSocket"],
                         frontend_section)
        self.assertFalse(monitor_uri_configured)
        frontend_helper.EXTRA_FRONTEND_SETTINGS = {}
    ...
    def test_config_default_front(self):
        ...
        frontend_helper.EXTRA_FRONTEND_SETTINGS['80'] = ["http-request add-header header1 value1"]
        frontend_helper.EXTRA_FRONTEND_SETTINGS['443'] = ["http-request add-header header2 value2"]
        cfg, monitor_uri_configured = config_default_frontend("ssl crt /certs/")
        self.assertEqual(OrderedDict([('frontend default_port_80',
        ...
        frontend_helper.EXTRA_FRONTEND_SETTINGS = OrderedDict()
        frontend_helper.EXTRA_FRONTEND_SETTINGS['2376'] = ["http-request add-header header1 value1"]
        cfg, monitor_uri_configured = config_default_frontend("ssl crt /certs/")
        ...
        frontend_helper.SKIP_FORWARDED_PROTO = ""
        frontend_helper.EXTRA_FRONTEND_SETTINGS = OrderedDict()
        frontend_helper.EXTRA_FRONTEND_SETTINGS['2375'] = ["http-request add-header header1 value1"]
        cfg, monitor_uri_configured = config_default_frontend("")
